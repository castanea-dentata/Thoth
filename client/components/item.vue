...
export default {
    ...
    computed: {
        ...
        item() {
            ...
            if (!this.$store.state.library || itemId === undefined) {
                return {};
            }
            return this.$store.state.library.getItemById(itemId) || {};
        },
        categoryItem() {
            const itemId = this.itemContainer.item.id;
    ...
    methods: {
        ...
        setDisplayPrice() {
            if (!this.priceError && this.item && typeof this.item.price === 'number') {
