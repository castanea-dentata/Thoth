...
const store = new createStore({
    ...
    mutations: {
        ...
        newList(state) {
            ...
            state.library.defaultListId = list.id;
            state.library.lists = [...state.library.lists];
        },
    ...
    actions: {
        async saveItem({ state }, itemId) {
            if (!state.library) return;
            const itemData = state.library.getItemById(itemId);
            if (itemData) {
                // Use fetchJson (already available) instead of axios (not imported)
                await fetchJson(`/api/item/${itemId}`, {
                    method: 'POST',
        ...
        async saveList({ state }, listId) {
            if (!state.library) return;
            const listData = state.library.getListById(listId);
            if (listData) {
                await fetchJson(`/api/list/${listId}`, {
                    method: 'POST',
